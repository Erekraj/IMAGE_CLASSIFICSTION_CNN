...
def cifar10_classification():
    ...
    if uploaded_file is not None:
        ...
        st.markdown(
            f"<h2 style='font-size:30px; color:blue;'>Predicted Class: {cifar10_classes[predicted_class]}</h2>", 
        unsafe_allow_html=True
        )
        st.markdown(
        f"<h3 style='font-size:25px; color:green;'>Confidence: {confidence * 100:.2f}%</h3>", 
        unsafe_allow_html=True
        )
